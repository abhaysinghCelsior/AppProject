...
function AddFundsScreen(props) {
  ...
  // Add InitialValue field, default to 0.0 or value from inp_fr_par if available
  const [initialValue, setInitialValue] = useState(inp_fr_par?.Funds?.InitialValue ?? 0.0);
  ...
  const addFund = () => {
    ...
    const funds_Data_Create = {
      ...
      InitialValue: initialValue, // Pass InitialValue to API
      totalFund,
      allocatedFund,
      unusedFund,
      MFPer,
